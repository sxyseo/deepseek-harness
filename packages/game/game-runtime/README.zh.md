...
| `captureFrame({ project, outputPath, scenePath?, width?, height?, engine? })` | 经选定引擎捕获项目的一帧，写出 PNG。 |
...
Provider 需要实现的抽象后端契约；它本身不是 Cordis service —— 注册表拥有其生命周期。`start` 是异步的，因为 provider 必须在 spawn 前解析引擎可执行文件。`sendInput` 是 M4 的输入表面；尚未实现它的 provider 必须抛出 `GameError` `GAME_CAPABILITY_UNAVAILABLE`，而不是伪造结果。
...
`GameError` 继承 `HarnessError`，带机器可路由的 code。共享 code：`GAME_DUPLICATE_RUNTIME`、`GAME_ENGINE_UNKNOWN`、`GAME_ENGINE_AMBIGUOUS`、`GAME_ENGINE_UNAVAILABLE`、`GAME_INVALID_REQUEST`、`GAME_EXECUTABLE_MISSING`、`GAME_PROCESS_UNKNOWN`、`GAME_CAPABILITY_UNAVAILABLE`、`GAME_QUERY_FAILED`、`GAME_CAPTURE_FAILED`。Provider 可添加自有 code；consumer 必须容忍它们。

## Model Experience

间接，经由模型可见的游戏工具（`dsh-tool-game`），它们拥有构建、运行、引擎日志、场景/资产查询与帧捕获的全部模型可见渲染；注册表自身不注册任何 prompt、schema 或结果文本。
...
- **输入投递已声明但未实现** — `sendInput` 存在于 seam 上以便 provider 逐步长入；尚无 Godot 实现或模型可见工具（M4 里程碑）。尚不存在的工具：`game_send_input`。
- **没有面向模型的停止工具** — 已启动的进程一直运行到会话/注册表销毁或后端将其终止；`registry.stop()` 仅是宿主 API。`game_stop` 工具推迟到试玩里程碑。
- **帧捕获由 provider 定义** — seam 返回 `{ imagePath, width, height }`；机制（以及 headless 渲染可用性）是每个 provider 自己文档化的选择。
